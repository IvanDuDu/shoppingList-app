...
if(respone.statusCode>=400){
  setState(() {
    _error='failed to fetch the data.pls try later';
  });
...
for( final item in listData.entries){
  final catgory =categories.entries.firstWhere((catItem)
...
final index=_groceryItems.indexOf(item);
  ...
  final url=Uri.https(
  'flutter-prep-df755-default-rtdb.asia-southeast1.firebasedatabase.app','shopping-list/${item.id}.json');
 ...
 if(respone.statusCode >=400){
  ...
  Widget build(BuildContext context) {
    Widget content= const Center(child:Text('Add more groceries'),);
